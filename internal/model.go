...
func (d resourceDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	...
	if server, ok := item.(Server); ok {
		...
		// Handle AppRunASG
		if index == m.Index() {
			str = selectedItemStyle.Render(fmt.Sprintf("> %-40s %-10s %d/%d-%d nodes",
				asg.Name,
				asg.Zone,
				asg.WorkerNodeCount,
				asg.MinNodes,
			...
			str = itemStyle.Render(fmt.Sprintf("  %-40s %-10s %d/%d-%d nodes",
				asg.Name,
				asg.Zone,
				asg.WorkerNodeCount,
				asg.MinNodes,
...
type model struct {
	...
	appRunDrilldownLevel    int    // 0: Cluster, 1: ASG, 2: LB
	appRunSelectedClusterID string // selected cluster ID for ASG list
